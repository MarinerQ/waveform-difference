...
def calculate_deltasq_kernal(sample_ID, Deltasq_list, samples, waveform_generator1, waveform_generator2, ifos):
    ...
    #h_EOB=waveform_generator1.frequency_domain_strain(parameters=inj_para)['cross']
    #h_IMR=waveform_generator2.frequency_domain_strain(parameters=inj_para)['cross']
...
if __name__ == '__main__':
    ...
    lambda_grid = np.linspace(0,5000,Ngrid1)
    #a_grid = np.linspace(-0.5,0.5,Ngrid2)
    a_grid = np.linspace(-0.2,0.2,Ngrid2)
    ...
    lambda_1 = np.reshape(np.meshgrid(lambda_grid, a_grid)[0], (Nsample,))
    lambda_2 = lambda_1
    a_1 = np.reshape(np.meshgrid(lambda_grid, a_grid)[1], (Nsample,))
    a_2 = a_1

    mass_1 = np.zeros(Nsample) + 1.4  
    mass_2 = np.zeros(Nsample) + 1.4  
    mass_ratio = np.zeros(Nsample) + 1.0
    ...
    waveform_arguments1 = dict(waveform_approximant="IMRPhenomD_NRTidal",
                            reference_frequency=50., minimum_frequency=30)

    '''
    waveform_arguments2 = dict(waveform_approximant="SEOBNRv4T_surrogate",#_surrogate
                            reference_frequency=50., minimum_frequency=30)
    '''
    waveform_arguments2 = dict(waveform_approximant="SEOBNRv4_ROM_NRTidal",
                            reference_frequency=50., minimum_frequency=30)
    ...
    savefilename = "paragrid_BNS_lowspin64k_H1_2NRT" + ".txt"
